...
class AmountValidator(Validator):
    ...
    """
    def __init__(self, min_amount: float = -float('inf'), max_amount: float = float('inf'), exclusive: bool = True) -> None:
    ...
    def __call__(self, key: str, value: float) -> float:
        if self.exclusive:
            if not (self.min_amount < value < self.max_amount):
                raise ValueError(f"{key.capitalize()} must be between {self.min_amount} and {self.max_amount}")
        else:
            if not (self.min_amount <= value <= self.max_amount):
                raise ValueError(f"{key.capitalize()} must be between {self.min_amount} and {self.max_amount}")
